...
- **Ground:** Servo GND must be connected to the controller GND.

## Firmware / Software
- **Platform:** PlatformIO (ESP32 Arduino framework)
- **Web Server:** AsyncWebServer + AsyncTCP
- **UI Storage:** LittleFS (`/data/www` served as static files)
- **Motion Control:** AccelStepper in DRIVER mode
- **Critical Setting:** `AccelStepper::setMinPulseWidth(10)` for reliable step pulses
- **Design Rule:** Motors must never be disabled by firmware (no enable switching logic).

## Current Goal
- Web-controlled plotter with stable motion.
- Two TB6600 drivers controlled via step/dir.
- MG995R reliably controlling the pen up/down action.
- No enable toggling, no random motor release.